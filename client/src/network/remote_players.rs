        ...
        for ps in states {
            ...
            for (rp, transform, mut interp) in &mut players {
                if rp.player_id == ps.player_id {
                    ...
                    interp.target_pos = Vec3::new(ps.x, ps.y, ps.z);
...
fn animate_remote_players(
    ...
    for (entity, interp, mut anims) in &mut remotes {
        ...
        if player_check.get(anims.player_entity).is_err() {
            if let Some(new_player) = find_animation_player(entity, &children, &player_check)
