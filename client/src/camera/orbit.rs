...
pub fn camera_follow(
    ...
    let Some((char_entity, char_transform, char_state, move_input)) = character_query.iter().next()
    else {
        return;
    };
    ...
    for (mut cam_transform, mut orbit) in &mut camera_query {
            ...
            && move_input.direction.y > -0.1
        {
            ...
            let move_dir = forward * move_input.direction.y + right * move_input.direction.x;
            if move_dir.length_squared() > 0.01 {
                orbit.last_move_yaw = move_dir.x.atan2(move_dir.z);
            }
        }

        // Re-enable auto-follow as soon as the character starts moving,
        // but not while the user is actively dragging (return_timer == 0 means dragging)
        if is_moving && !orbit.auto_follow && orbit.return_timer > 0.0 {
            orbit.auto_follow = true;
        }
        ...
        if let Some(ref terrain) = terrain {
            let terrain_y = heightmap::sample_height(&terrain.heightmap, final_pos.x, final_pos.z);
        ...
        if orbit.needs_snap {
            ...
            cam_transform.translation = cam_transform
                .translation
                .lerp(final_pos, lerp_speed.min(1.0));
        }
