...
pub fn generate_terrain_texture(heightmap: &HeightmapData) -> Image {
    ...
    for z in 0..d {
        for x in 0..w {
            ...
            let (r, g, b) = terrain_color(height_ratio, slope, patch_noise, detail_noise, micro_noise);
    ...
    Image::new(
        Extent3d { width: w, height: d, depth_or_array_layers: 1 },
        TextureDimension::D2,
        data,
...
fn compute_slope(heightmap: &HeightmapData, x: u32, z: u32, step: f32) -> f32 {
    ...
    let h_left = if x > 0 { heightmap.heights[idx(x - 1, z)] } else { h_center };
    let h_right = if x < w - 1 { heightmap.heights[idx(x + 1, z)] } else { h_center };
    let h_down = if z > 0 { heightmap.heights[idx(x, z - 1)] } else { h_center };
    let h_up = if z < d - 1 { heightmap.heights[idx(x, z + 1)] } else { h_center };

    Vec3::new(h_left - h_right, 2.0 * step, h_down - h_up).normalize().y
}
...
fn terrain_color(h: f32, slope: f32, patch: f32, detail: f32, micro: f32) -> (f32, f32, f32) {
    ...
    if slope < 0.72 {
        ...
        let rock = lerp3(&rock_gray, &rock_warm, detail);
        let ground = height_ground_color(h, patch, &grass_lush, &grass_dark, &meadow, &grass_yellow, &dirt_light, &dirt_dark);
