...
impl Plugin for VegetationPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, spawn_vegetation.after(crate::terrain::setup_terrain))
...
fn spawn_vegetation(
    ...
    let spruce_scene: Handle<Scene> = asset_server
        .load(GltfAssetLabel::Scene(0).from_asset("models/pp_spruce2/pp_spruce2.glb"));
    let pine_scene: Handle<Scene> = asset_server
        .load(GltfAssetLabel::Scene(0).from_asset("models/polypizza_pine/pine.glb"));
    ...
    let shrub_scene: Handle<Scene> = asset_server
        .load(GltfAssetLabel::Scene(0).from_asset("models/alpine_shrub.glb"));

    let dirt_scene: Handle<Scene> = asset_server
...
fn distance_cull_vegetation(
    ...
    mut veg_query: Query<(&Transform, &mut Visibility), (With<VegetationType>, Without<CharacterMarker>)>,
) {
    let Some(char_tf) = character_query.iter().next() else { return };
